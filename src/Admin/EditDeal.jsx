...
const EditDeal = () => {
  ...
  const fetchDeal = async () => {
    try {
      ...
      const mockDeal = {
        ...
        description: 'Up to 50% off on selected electronics items including laptops, smartphones, and accessories.',
        discountType: 'percentage',
        discountValue: 50,
        ...
        maxUsage: 100,
        minOrderAmount: 50,
        terms: 'Valid only on selected items. Cannot be combined with other offers. Limited stock available.'
      };
      
      setFormData({
        title: mockDeal.title,
        description: mockDeal.description,
        discountType: mockDeal.discountType,
        discountValue: mockDeal.discountValue.toString(),
        originalPrice: mockDeal.originalPrice.toString(),
        dealPrice: mockDeal.dealPrice.toString(),
        category: mockDeal.category,
        productIds: mockDeal.productIds,
        featuredImage: mockDeal.featuredImage,
        status: mockDeal.status,
        startDate: mockDeal.startDate,
        endDate: mockDeal.endDate,
        maxUsage: mockDeal.maxUsage.toString(),
        minOrderAmount: mockDeal.minOrderAmount.toString(),
        terms: mockDeal.terms
      });
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Deal updated:', { id, ...formData });
      navigate('/admin/deals');
    } catch (error) {
      ...
      <div className="page-header">
        ...
        <div className="page-actions">
          ...
          <button className="btn btn-primary" onClick={handleSubmit} disabled={saving}>
            <FaSave /> {saving ? 'Saving...' : 'Update Deal'}
          </button>
        </div>
      </div>
